...
public class Tree_TED {
	
	HashMap<Integer, Node_TED> nodeMap;
	Node_TED root;
	int k; // Record number of levels
	HashMap<Integer, ArrayList<Node_TED>> AT; // Record node array per level
	int[] maxDegree; // Record maximal degree of the nodes per level
	
	public Tree_TED(String FILENAME){
		this.nodeMap = new HashMap<Integer, Node_TED>();
		this.k = 0;
		this.AT = new HashMap<Integer, ArrayList<Node_TED>>();
		...
		try{
			...
			while((line = bufferedReader.readLine()) != null) {
				...
				if(str[0].equals("k")) {
					...
					degrees = str[1].split(",");
					for(int i = 0; i <= k; i++) {
						AT.put(i, new ArrayList<Node_TED>());
					...
					if(!nodeMap.containsKey(src)){
						this.nodeMap.put(src, new Node_TED(src));
					}
					if(!nodeMap.containsKey(dst)){
						this.nodeMap.put(dst, new Node_TED(dst, this.nodeMap.get(src)));
					}			
	...
	public HashSet<Node_TED> getNodeSet(){
		HashSet<Node_TED> nodeSet = new HashSet<Node_TED>();
